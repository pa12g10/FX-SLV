...
def render_single_barrier_section():
    ...
    with col2:
        # Suggest barrier based on type
        if "Up" in barrier_type:
            default_barrier = spot_fx * 1.15
        else:
            default_barrier = spot_fx * 0.85
    ...
    # Visual representation of option setup
    ...
    if 'sb_fd_price' in st.session_state or 'sb_mc_results' in st.session_state:
        ...
        with tab1:
            ...
            if 'sb_mc_results' in st.session_state:
                ...
                with col2:
                    st.metric("Breach Probability", f"{mc_results['breach_probability']:.2%}")
                with col3:
                    ci_95 = 1.96 * mc_results['std_error']
                    st.metric("95% Confidence Interval", f"±{ci_95:.6f}")
        ...
        with tab3:
            ...
            for s in spot_range:
                ...
                if barrier_type == "UpOut" and s >= barrier:
                    payoff = 0
                elif barrier_type == "DownOut" and s <= barrier:
                    payoff = 0
                elif barrier_type == "UpIn" and s < barrier:
                    payoff = 0
                elif barrier_type == "DownIn" and s > barrier:
                    payoff = 0
                else:
                    # Calculate vanilla payoff if barrier not breached
            ...
            levels_data = [
                {"Level": "Current Spot", "Value": spot_fx, "Distance": "0.00%"},
                {"Level": "Strike", "Value": strike, "Distance": f"{((strike/spot_fx - 1)*100):.2f}%"},
                {"Level": "Barrier", "Value": barrier, "Distance": f"{((barrier/spot_fx - 1)*100):.2f}%"}
            ]
            ...
            with col2:
                barrier_distance = abs((barrier - spot_fx) / spot_fx) * 100
                st.metric("Distance to Barrier", f"{barrier_distance:.2f}%")
            
            with col3:
                if 'sb_mc_results' in st.session_state:
                    breach_prob = st.session_state.sb_mc_results['breach_probability']
                    st.metric("Breach Probability", f"{breach_prob:.2%}")
