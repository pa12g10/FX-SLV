...
# ---------------------------------------------------------------------------
def delta_to_strike(delta, S, T, rd, rf, sigma, is_put=False):
    """
    Invert GK delta to strike.
    delta  : absolute delta value (e.g. 0.25 for 25D)
    is_put : True for put deltas (25P, 10P)
    """
    from scipy.stats import norm
    sign = -1.0 if is_put else 1.0
    # phi(d1) = delta * exp(rf*T)  for call; -phi(d1) = delta * exp(rf*T) for put
    d1_target = sign * norm.ppf(sign * delta * np.exp(rf * T))
    strike = S * np.exp(-d1_target * sigma * np.sqrt(T) + (rd - rf + 0.5 * sigma**2) * T)
    return strike
...
FX_OPTION_INSTRUMENTS = pd.DataFrame({
    ...
    # Notional in EUR millions (typical interbank)
...
def _build_vol_surface_from_instruments(instruments_df, spot, rd, rf):
    ...
    for _, row in instruments_df.iterrows():
        ...
        # For ATM (delta = 0.50 call), strike ~ F * exp(0.5 * sigma^2 * T)
        if row["Instrument"] == "ATM":
            F = spot * np.exp((rd - rf) * T)
            strike = F * np.exp(0.5 * vol**2 * T)
        else:
            try:
                strike = delta_to_strike(abs_delta, spot, T, rd, rf, vol, is_put=is_put)
            except Exception:
                # Fallback: use ATM as strike
                F = spot * np.exp((rd - rf) * T)
                strike = F * np.exp(0.5 * vol**2 * T)
...
def render_fx_slv_section():
    ...
    # Pull domestic/foreign zero rates for delta → strike conversion
    try:
        rd = fx_curves.usd_curve.zeroRate(0.5, ql.Continuous).rate()
        rf = fx_curves.eur_curve.zeroRate(0.5, ql.Continuous).rate()
    except Exception:
        rd, rf = 0.053, 0.035   # USD/EUR fallback (March 2026 approx)

    st.markdown("---")

    # -----------------------------------------------------------------------
    # 1.  FX OPTIONS INSTRUMENT TABLE  (replaces the old vol surface 3D plot)
    # -----------------------------------------------------------------------
    ...
    # Derive strike-based vol surface for the model
    vol_surface_df = _build_vol_surface_from_instruments(instruments_df, spot_fx, rd, rf)

    st.markdown("---")

    # -----------------------------------------------------------------------
    # 2a.  VOLATILITY SMILES IN DELTA NOTATION
    # -----------------------------------------------------------------------
    ...
    if st.checkbox("Show Volatility Smile Plots", value=True, key="fx_smile_plot_check"):
        ...
        # x-axis labels in market order (put wing → ATM → call wing)
        delta_labels_ordered = ["10D Put", "25D Put", "ATM", "25D Call", "10D Call"]
        delta_x              = [-0.10,    -0.25,     0.50,  0.25,       0.10]   # numeric x positions
        ...
        for i, T in enumerate(unique_expiries):
            ...
            # Map instrument name to x position
            inst_to_x = dict(zip(delta_labels_ordered, delta_x))
            xs, ys = [], []
            for lbl in delta_labels_ordered:
                row = subset[subset["Instrument"] == lbl]
                if not row.empty:
                    xs.append(inst_to_x[lbl])
            ...
            fig_delta.add_trace(go.Scatter(
                ...
                text=delta_labels_ordered[:len(xs)],
            ))
        ...
        # ATM reference line
        fig_delta.add_vline(x=0.50, line_dash="dot", line_color="grey",
                            annotation_text="ATM (50D Call)", annotation_position="top right")

        st.plotly_chart(fig_delta, use_container_width=True, key="fx_smile_delta")

        # --- Plot B: FX Rate (Strike) space ---
        st.write("**Panel 2 – FX Rate Space (Delta Converted to EUR/USD Strikes)**")
        st.caption(
            "Same smiles plotted against the implied EUR/USD strike corresponding to each delta. "
            "Useful for seeing the smile relative to spot and forward levels."
        )
        ...
        for i, T in enumerate(unique_expiries):
            ...
            # Sort by strike for clean line
            sorted_pairs = sorted(zip(strike_xs, vol_ys, hover_texts), key=lambda x: x[0])
            if not sorted_pairs:
                continue
            xs_sorted, ys_sorted, ht_sorted = zip(*sorted_pairs)

            color = colors[i % len(colors)]
            fig_strike.add_trace(go.Scatter(
                x=list(xs_sorted), y=list(ys_sorted),
                mode="markers+lines",
                name=tenor_label,
                marker=dict(size=9, color=color),
                line=dict(width=2, color=color),
        ...
        fig_strike.add_vline(x=fwd_1y, line_dash="dot", line_color="grey",
    ...
    with col1:
        v0 = st.number_input("Initial Variance (v0)", value=0.014, format="%.4f",
                             help="Initial variance (vol^2). Default: 0.014 = 12% vol", key="fx_slv_v0")
        kappa = st.number_input("Mean Reversion (κ)", value=2.0, format="%.4f",
                                help="Mean reversion speed of variance.", key="fx_slv_kappa")
        theta = st.number_input("Long-term Variance (θ)", value=0.014, format="%.4f",
                                help="Long-term variance level.", key="fx_slv_theta")

    with col2:
        sigma = st.number_input("Vol-of-Vol (σ)", value=0.4, format="%.4f",
                                help="Volatility of variance. FX typically 0.3–0.5", key="fx_slv_sigma")
        rho = st.number_input("Correlation (ρ)", value=-0.6, min_value=-1.0, max_value=1.0,
                              format="%.4f",
                              help="Correlation between spot and volatility.", key="fx_slv_rho")
        ...
        with st.spinner("Calibrating FX-SLV model to volatility surface..."):
            try:
                if calibration_mode == "Reduced Set (Recommended)":
                    ...
                    for idx, expiry in enumerate(unique_expiries_calib[:2]):
                        ...
                        if not expiry_data.empty:
                            ...
                            if atm_idx < len(strikes_sorted) - 1:
                ...
                # Store instruments used so we can label the market vs model plot
    ...
    if st.session_state.fx_slv_model is not None:
        ...
        if results:
            ...
            with tab1:
                ...
                if rmse < 10:
                    quality_msg, quality_color = "🌟 Excellent calibration quality (RMSE < 10 bps)", "green"
                elif rmse < 20:
                    quality_msg, quality_color = "✅ Good calibration quality (RMSE < 20 bps)", "blue"
                elif rmse < 30:
                    quality_msg, quality_color = "⚠️ Acceptable calibration (RMSE < 30 bps)", "orange"
                else:
                    quality_msg, quality_color = "❌ Poor calibration (RMSE > 30 bps). Try reduced set mode.", "red"
            ...
            # -------------------------------------------------------------------
            # NEW TAB: Market vs Model Prices (arbitrage check)
            # -------------------------------------------------------------------
            with tab2:
                st.write("**Market vs Model Option Prices – Arbitrage Check**")
                st.info(
                    "💡 A well-calibrated, arbitrage-free model should have model prices that lie "
                    "on or very close to the 45° line (market = model). Points far from the line "
                    "indicate mis-pricing; systematic bias above/below suggests calendar or butterfly arbitrage."
                )

                errors_df = results['pricing_errors']

                # ---- Scatter: market price vs model price ----
                # Label each point with its tenor and instrument if we have the mapping
                inst_df_used = st.session_state.get('fx_slv_instruments_used', instruments_df)

                # Build label list aligned with errors_df
                labels = []
                tenor_labels = []
                for _, row in errors_df.iterrows():
                    K = row['strike']
                    T = row['expiry']
                    # Try to match to instrument table
                    match = inst_df_used[
                        (np.abs(inst_df_used['Expiry (Years)'] - T) < 1e-4)
                    ]
                    if not match.empty:
                        # Pick closest delta-derived instrument label
                        best_row = match.iloc[0]
                        lbl = f"{best_row['Tenor']} {best_row['Instrument']}"
                        tnr = best_row['Tenor']
                    else:
                        lbl = f"K={K:.4f} T={T:.2f}Y"
                        tnr = f"{T:.2f}Y"
                    labels.append(lbl)
                    tenor_labels.append(tnr)
                ...
                # ---- Per-instrument price error bar chart ----
                st.write("**Price Error by Instrument (Model − Market, %)**")

                # Colour bars: green inside ±2%, red outside
                bar_colors = [
                    "#2ca02c" if abs(v) <= 2.0 else "#d62728"
                ...
                if n_outside == 0:
                    st.success(
                        f"✅ All {n_total} instruments price within ±2% tolerance. "
                        "No pricing arbitrage detected."
                    )
                else:
                    st.warning(
                        f"⚠️ {n_outside}/{n_total} instruments price outside ±2% tolerance. "
                        "Review calibration or consider tighter convergence criteria."
                    )
                    ...
                    with st.spinner("Generating Monte Carlo paths..."):
                        ...
                        for i in range(min(num_paths, 1000)):
                            fig_vol.add_trace(go.Scatter(
                                x=times, y=vol_paths_pct[:, i],
                                mode='lines', line=dict(width=0.5),
                                opacity=0.3, showlegend=False, hoverinfo='skip',
                            ))
                        mean_vol = vol_paths_pct.mean(axis=1)
                        fig_vol.add_trace(go.Scatter(
                            x=times, y=mean_vol, mode='lines',
