...
from MarketData.market_data import get_eval_date, get_fx_option_instruments
...
def _plot_smile_delta(instruments_df):
    ...
    for i, T in enumerate(unique_expiries):
        ...
        for pillar in DELTA_PILLAR_ORDER:
            ...
            bids.append(float(r.iloc[0]["Bid Vol (%)"]))
            asks.append(float(r.iloc[0]["Ask Vol (%)"]))
...
def _plot_market_vs_model(errors_df, instruments_df):
    ...
    edf = errors_df.copy()
    edf["tenor_label"] = [
        tenor_map.get(round(float(row["expiry"]), 6), f"{row['expiry']:.2f}Y")
...
def _plot_smile_overlay(errors_df, instruments_df, tenor="1Y"):
    ...
    for pillar in DELTA_PILLAR_ORDER:
        ...
        bids_mkt.append(float(r.iloc[0]["Bid Vol (%)"]))
        asks_mkt.append(float(r.iloc[0]["Ask Vol (%)"]))
        htexts.append(pillar)
    ...
    for pillar in DELTA_PILLAR_ORDER:
        ...
        if pillar == "ATM":
            ...
            try:
                K_pillar = delta_to_strike(
                    abs(delta_val), 1.17, T_val, rd_approx, rf_approx, vol_dec, is_put=is_put
                )
            except Exception:
                continue
        if len(edf_t) == 0:
...
def render_fx_slv_section():
    ...
    # Load from MarketData -- users can override via the table editor
    instruments_df = st.data_editor(
        get_fx_option_instruments(),
        ...
        if results:
            ...
            with tab_s3:
                ...
                st.caption(
                    ...
                    "Red dashed = Heston model. Any gap is the actual calibration residual."
                )
                    ...
                    with st.spinner("Generating paths..."):
                        ...
                        for i in range(min(num_paths, 1000)):
                            fig_vol.add_trace(go.Scatter(
                                x=times, y=vp[:, i], mode='lines',
                                line=dict(width=0.5), opacity=0.3, showlegend=False, hoverinfo='skip'))
                        fig_vol.add_trace(go.Scatter(
                            x=times, y=vp.mean(axis=1),
